(function() {
  ...
  var d3v3 = window.d3v3, topojson = window.topojson;
  ...
  */
  ...
  function addContainer( element, height, width ) {
    this.svg = d3v3.select( element ).append('svg')
    ...
    if (this.options.responsive) {
      d3v3.select(this.options.element).style({'position': 'relative', 'padding-bottom': (this.options.aspectRatio*100) + '%'});
      d3v3.select(this.options.element).select('svg').style({'position': 'absolute', 'width': '100%', 'height': '100%'});
      d3v3.select(this.options.element).select('svg').select('g').selectAll('path').style('vector-effect', 'non-scaling-stroke');
  ...
  function setProjection( element, options ) {
    ...
    if ( options.scope === 'usa' ) {
      projection = d3v3.geo.albersUsa()
        .scale(width)
        .translate([width / 2, height / 2]);
    }
    else if ( options.scope === 'world' ) {
      projection = d3v3.geo[options.projection]()
  ...
  function addStyleBlock() {
    if ( d3v3.select('.datamaps-style-block').empty() ) {
      d3v3.select('head').append('style').attr('class', 'datamaps-style-block')
  ...
  function handleGeographyConfig () {
    ...
    if ( options.highlightOnHover || options.popupOnHover ) {
      svg.selectAll('.datamaps-subunit')
        .on('click', function(geography, d) {


            //get selected Name Property
            options.clickedCountry = geography.properties.name;
            d3v3.selectAll('.datamaps-subunit').style('fill', 'rgba(200,200,200,0.9)');
            var $this = d3v3.select(this);

            $this
                .style('fill', options.selectedFill)
        ...
        .on('mouseout', function() {
          ...
          d3v3.selectAll('.datamaps-hoverover').style('display', 'none');
        });
    }
  ...
  function addLegend(layer, data, options) {
    ...
    var hoverover = d3v3.select( this.options.element ).append('div')
      ...
      .enter()
        ...
        .attr('d', function(datum) {
            ...
            if (options.greatArc) {
              ...
              var greatArc = d3v3.geo.greatArc()
  ...
  ***************************************/
  ...
  function Datamap( options ) {
    ...
    if ( typeof d3v3 === 'undefined' || typeof topojson === 'undefined' ) {
      throw new Error('Include d3v3v3.js (v3.0.3 or greater) and topojson on this page before creating a new map');
   }
    ...
    if ( d3v3.select( this.options.element ).select('svg').length > 0 ) {
  ...
  Datamap.prototype.resize = function () {
    ...
    if (options.responsive) {
      var newsize = options.element.clientWidth,
          oldsize = d3v3.select( options.element).select('svg').attr('data-width');

      d3v3.select(options.element).select('svg').selectAll('g').attr('transform', 'scale(' + (newsize / oldsize) + ')');
    }
  }
  ...
  Datamap.prototype.draw = function() {
    ...
    if ( options.geographyConfig.dataUrl ) {
      d3v3.json( options.geographyConfig.dataUrl, function(error, results) {
    ...
    return this;
      ...
      function draw (data) {
        ...
        if ( self.options.dataUrl ) {
          ...
          d3v3[self.options.dataType](self.options.dataUrl, function(data) {
        ...
        if ( self.options.geographyConfig.popupOnHover || self.options.bubblesConfig.popupOnHover) {
          hoverover = d3v3.select( self.options.element ).append('div')
  ...
  ***************************************/
  ...
  Datamap.prototype.updatePopup = function (element, d, options) {
    ...
    element.on('mousemove', function() {
      var position = d3v3.mouse(self.options.element);
      d3v3.select(self.svg[0][0].parentNode).select('.datamaps-hoverover')
    ...
    d3v3.select(self.svg[0][0].parentNode).select('.datamaps-hoverover').style('display', 'block');
  };
  ...
  if (typeof exports === 'object') {
    d3v3 = require('lib/d3v3v3');
    topojson = require('topojson');
    module.exports = Datamap;
  }
  else if ( typeof define === "function" && define.amd ) {
    define( "datamaps", ["require", "lib/d3v3v3", "topojson"], function(require) {
      d3v3 = require('lib/d3v3v3');
      topojson = require('topojson');
